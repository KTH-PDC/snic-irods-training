...
\huge{\textbf{BEGINNER TRAINING WITH iRODS 4.1.10}}
\textbf{SNIC iRODS Training Step 1}

\vspace{8mm}
\textbf{Lund, Sweden}

\textbf{August 21, 2017}



\vspace{5mm}
\centerline{\includegraphics[width=9cm]{SNIClogo}}
...
\begin{center}
	\begin{tabular}{ |l|l| }
		...
		\hline
		iinit & Log on \\
		iexit & Exits iRODS session \\
		iexit full & Exits and removes the scrambled password  \\
		ienv & Display current irods environment  \\
		ihelp &	List of icommands \\
		ipasswd	& Change iRODS password \\
		iuserinfo &	Show user information \\
...
With the \texttt{ienv} command you can retrieve some information on the iRODS system you are working on.
...
\begin{center}
	\begin{tabular}{ |l|l| }
		...
		\hline
          ...
          iadmin lu &  list user info; details if name entered \\
          iadmin lg & list group info  \\
          iadmin lgd & list group details  \\
		\hline
	\end{tabular}
\end{center}
...
Let's use iadmin to create a second resource, \texttt{newResc}, of the type unixfilesystem, on the host \texttt{X.X.X.X}, and mounted at \texttt{/var/lib/irods/new\_vault}. This will be a single line of text on your screen
...
\begin{lstlisting}
$ wget https://github.com/irods/irods_training/raw/ugm2017/beginner/training_jpgs.zip
...
\begin{lstlisting}
$ iput -rK /home/ubuntu/training_jpgs
\end{lstlisting}
...
\begin{center}
	\begin{tabular}{ |l|l| }
		...
		\hline
         ...
         icp & Creates a new copy that will not inherit any metadata \\		
         \hline
	\end{tabular}
\end{center}
...
\begin{lstlisting}
iquest "select sum(DATA_SIZE), COLL_NAME where COLL_NAME like '/tempZone/home/alice/tre%'"
\end{lstlisting}

This command sums the sizes of all data in our collection starting with "tre". The command \texttt{iquest} already knows some keywords specific to the iRODS environment. You can list them with
...
Let's test it. Save the example rule below in a file called HelloWorld.r
...
A user may manually execute a rule that operates on data that she has access to by using the \texttt{irule} command; for example, to verify the checksums of data of a certain age or to test and execute the HelloWWorld rule: 


\begin{lstlisting}
irule -F HelloWorld.r
\end{lstlisting}

or to write to iRODS server log,

\begin{lstlisting}[language=python]
HelloWorld {
	writeLine("serverLog", "Hello World2Log");
}
input null
output ruleExecOut
\end{lstlisting}

execute and check your iRODS server log (usually  /var/lib/irods/iRODS/server/log/rodsLogDATE)

\begin{lstlisting}
irule -F HelloWorld.r
tail -50 /var/lib/irods/iRODS/server/log/rodsLog.2017.08.21
\end{lstlisting}
...
We can also pass arguments and ouput. Try it by following this example:
...
iRODS provides a default rule base in /etc/irods/core.re. These rules can be employed and called by your own rules. In the following example we make use of the printHello rule from the core.re:
...
We can also schedule, delay rules and execute rules regularly.
...
The function delay delayes the execution by 1 minute and restarts the rule automatically every 5 minutes. With \texttt{iqstat} we can check the status of the rule. The output of the rule is written to the rodsLog file in /var/lib/irods/iRODS/server/log/reLogDATE.
...
\section{The Example Rule: Harvesting and Applying Metadata}

We will use the rule that could help Alice save time by automating metadata annotation for her. 

iRODS microservices used in this example are provided by Dell EMC Metalnx and licensed under the Apache License, Version 2.0.  
Metalnx MSI package (\url{https://github.com/Metalnx/metalnx-msi}) provides a few iRODS microservices implementations for automatic metadata extraction. 

\begin{lstlisting}
$ cd ~/packages
$ sudo dpkg -i metalnx-msi-plugins-4.1.10-1.1.0-50.deb irods-dev-4.1.10-ubuntu14-x86_64.deb
$ sudo apt-get -f install
$ sudo apt-get install imagemagick
$ sudo su - irods
\end{lstlisting}

At this point, microservices have been extracted to ../plugins/microservices directory.

\begin{lstlisting}
$ ls plugins/microservices/
libmsiget_illumina_meta.so     libmsiobjput_irods.so
libmsiobjget_http.so	       libmsiobjput_mdbam.so
libmsiobjget_irods.so	       libmsiobjput_mdmanifest.so
libmsiobjget_microservices.so  libmsiobjput_mdvcf.so
libmsiobjget_slink.so	       libmsiobjput_populate.so
libmsiobjget_version.so        libmsiobjput_slink.so
libmsiobjjpeg_extract.so       libmsisync_to_archive.so
libmsiobjput_http.so	       libmsi_update_unixfilesystem_resource_free_space.so
\end{lstlisting}

An implementation of the particular PEP, we are going to look at, already exists in the iRODS Rule Language rulebase that is shipped with iRODS (\texttt{core.re});
We need the \texttt{acPostProcForPut}) PEP to perform metadata to be harvested and applied to data objects for later discovery by SPS users.

Let's check metadata contained in file we downloaded previously from iRODS:

\begin{lstlisting}
$ cd ~/downloads
$ identify -verbose download/lemur.jpg
Image: download/lemur.jpg
Format: JPEG (Joint Photographic Experts Group JFIF format)
Class: DirectClass
Geometry: 3264x1836+0+0
Resolution: 240x240
Print size: 13.6x7.65
Units: PixelsPerInch
Type: TrueColor
Endianess: Undefined
Colorspace: sRGB
...
min: 0 (0)
max: 255 (1)
mean: 109.328 (0.428736)
standard deviation: 49.6914 (0.194868)
kurtosis: 0.0820419
skewness: 0.669963
Green:
min: 1 (0.00392157)
max: 255 (1)
mean: 132.465 (0.519472)
standard deviation: 48.3139 (0.189466)
kurtosis: -0.375999
skewness: 0.0457503
Blue:
min: 0 (0)
max: 255 (1)
mean: 76.3599 (0.299451)
standard deviation: 68.6086 (0.269053)
kurtosis: -0.364264
skewness: 0.766702
Image statistics:
Overall:
min: 0 (0)
max: 255 (1)
mean: 106.051 (0.415886)
standard deviation: 56.3045 (0.220802)
kurtosis: 0.309981
skewness: 0.218912
...
Orientation: TopLeft
Properties:
date:create: 2017-06-27T16:52:57+00:00
date:modify: 2017-06-27T16:52:57+00:00
...
exif:DateTime: 2012:07:30 15:10:41
exif:DateTimeDigitized: 2012:07:29 10:15:58
exif:DateTimeOriginal: 2012:07:29 10:15:58
...
exif:FocalLengthIn35mmFilm: 102
exif:GainControl: 2
exif:ImageLength: 2176
exif:ImageWidth: 3264
exif:ISOSpeedRatings: 311
exif:JPEGInterchangeFormat: 874
exif:JPEGInterchangeFormatLength: 8846
exif:LightSource: 0
exif:Make: NIKON
exif:MaxApertureValue: 30/10
exif:MeteringMode: 5
exif:Model: COOLPIX P50
exif:Orientation: 1
...
signature: 53f67d99f9f1ed07d2998e5d27a3afdc250fb1d58b0519b4d29d8f88535c2f98
xapMM:DerivedFrom: 
Profiles:
Profile-8bim: 10338 bytes
Profile-exif: 9726 bytes
...
unknown[2,0]: 
Profile-xmp: 7420 bytes
Artifacts:
filename: download/lemur.jpg
verbose: true
Tainted: True
Filesize: 1.312MB
Number pixels: 5.993M
Pixels per second: 49.94MB
User time: 0.120u
Elapsed time: 0:01.119
Version: ImageMagick 6.7.7-10 2017-05-26 Q16 http://www.imagemagick.org
\end{lstlisting}

And we can upload the file into iRODS Zone and check associated metadata: 

\begin{lstlisting}
$ iput lemur.jpg testlemur.jpg
$ imeta ls -d testlemur.jpg
AVUs defined for dataObj testlemur.jpg:
None
\end{lstlisting}

Now it is the time to prepare automating metadata annotation process.
The file \texttt{/etc/irods/core.re} will be edited with the new behavior like this:
...
acPostProcForPut {
	ON($objPath like "\*.jpg") {
		msiobjjpeg_extract($objPath, $filePath);}
}
...
\end{lstlisting}

NOTE: \texttt{core.re} may change from release to release, so it might be overwritten during an upgrade. 
On production iRODS instance you should write the rule into an accessory rulebase (i.e., a file that contains a rule or rules but is not \texttt{core.re}) in \texttt{/etc/irods} and then tell iRODS to read this file by editing the \\ \texttt{/etc/irods/server\_config.json} file. When we do this, the rules in accessory rulebases, will combine to take precedence over the existing PEP in \texttt{core.re}.

Now let's try again to upload the file to iRODS and look at the metadata that has been applied to \texttt{lemur.jpg}:

\begin{lstlisting}
$ iput lemur.jpg testlemur2.jpg
$ imeta ls -d testlemur2.jpg
AVUs defined for dataObj testlemur2.jpg:
...
units: 
----
attribute: PixelYDimension
value: 1836
units: 
----
attribute: DateTimeDigitized
value: 2012:07:29 10:15:58
units: 
----
attribute: PixelXDimension
value: 3264
units: 
----
...
units: 
----
attribute: ResolutionUnit
value: 2
units: 
----
attribute: DateTime
value: 2012:07:30 15:10:41
units: 
----
...
This simple example demonstrates automated metadata harvesting and gives idea of possible applications. Now you're ready for the advanced training!
